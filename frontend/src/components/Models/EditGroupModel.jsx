...
const EditGroupModel = ({ setSelectEdit, selectEdit }) => {
  const [groupTourName, setGroupTourName] = useState(selectEdit.name);
  const [totalSeats, setTotalSeats] = useState(selectEdit.total);
  const [availableSeats, setAvailableSeats] = useState(selectEdit.available);
  const [flyer, setFlyer] = useState(null);
  const { user } = useContext(AuthContext);

  const [loading, setLoading] = useState(false);
  const [countriesArray, setCountriesArray] = useState(Countries);
  const [country, setCountry] = useState(selectEdit.country);
  const [showCountries, setShowCountries] = useState(false);
  const [showCities, setShowCities] = useState(false);
  const [cities, setCities] = useState(selectEdit.mainCities);
  ...
  return (
    <>
      ...
      {loading ? (
        ...
        <>
          <div className=" w-[520px] bg-slate-50  px-10 py-4  rounded-[10px]  shadow-lg shadow-black/25 relative">
            <div
              className=" absolute top-[-10px] right-[-10px] bg-red-500 text-white rounded-full w-7 h-7 flex justify-center items-center cursor-pointer hover:bg-red-700"
